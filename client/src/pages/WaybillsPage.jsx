      ...
      <Modal isOpen={modal} onClose={()=>{setModal(false);setForm(EMPTY);setEditId(null);}} title={editId?"עריכת תעודה":"הוספת תעודת משלוח"} size="md">
        <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div><label style={S.label}>חברה / מוסד</label><input type="text" value={val("clientName")} onChange={e=>set("clientName",e.target.value)} list="wb-clients" required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/><datalist id="wb-clients">{allClients.map(c=><option key={c} value={c}/>)}</datalist></div>
            <div style={{gridColumn:"1/-1"}}><label style={S.label}>כתובת משלוח</label><input type="text" value={val("location")} onChange={e=>set("location",e.target.value)} list="wb-locations" required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/><datalist id="wb-locations">{allLocations.map(l=><option key={l} value={l}/>)}</datalist></div>
            <div style={{gridColumn:"1/-1"}}><label style={S.label}>תיאור מוצר</label><input type="text" value={val("name")} onChange={e=>set("name",e.target.value)} list="wb-names" required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/><datalist id="wb-names">{allNames.map(n=><option key={n} value={n}/>)}</datalist></div>
            <div><label style={S.label}>כמות</label><input type="number" min="1" value={val("quantity")} onChange={e=>set("quantity",e.target.value)} required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/></div>
