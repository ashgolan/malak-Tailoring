...
const LABELS = {
  ...
  providers:        "ספקים",
  contacts:         "אנשי קשר",
  tasks:            "משימות חברות",   // ✅ חדש
  companies:        "חברות ומוסדות", // ✅ חדש
  users:            "משתמשים",
  settings:         "הגדרות",
};
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(135deg,#1a1a2e 0%,#16213e 50%,#0f3460 100%)", display: "flex", alignItems: "center", justifyContent: "center", padding: 20, direction: "rtl", fontFamily: "Assistant,Arial,sans-serif" }}>
      <div style={{ width: "100%", maxWidth: 520 }}>
        ...
        {step === 1 && (
          <div style={cardStyle}>
            <div style={{ background: "rgba(239,68,68,0.15)", border: "1px solid rgba(239,68,68,0.3)", borderRadius: 10, padding: "12px 16px", fontSize: 13, color: "#fca5a5" }}>
            ...
            <form onSubmit={handleCheckKey} style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <div>
                ...
                <input type="password" value={key} onChange={e => setKey(e.target.value)} placeholder="הזן מפתח חירום סודי" required style={inputStyle} />
              </div>
              ...
              <button type="submit" disabled={loading} style={{ padding: "12px", background: "linear-gradient(135deg,#ef4444,#dc2626)", color: "#fff", border: "none", borderRadius: 10, fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
                {loading ? "מאמת..." : "המשך"}
              </button>
            </form>
              ...
              <div style={{ background: "rgba(255,255,255,0.06)", borderRadius: 10, padding: "14px 16px" }}>
                {backupInfo.fileSize && (
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 8 }}>
                    גודל: <strong style={{ color: "rgba(255,255,255,0.8)" }}>{backupInfo.fileSize}</strong>
                    <span style={{ marginRight: 8, color: "#6ee7b7" }}>— ZIP</span>
                  </div>
                )}
                {backupInfo.exportedAt && (
                  <div style={{ fontSize: 12, color: "rgba(255,255,255,0.5)", marginBottom: 8 }}>
                    תאריך: <strong style={{ color: "rgba(255,255,255,0.8)" }}>{new Date(backupInfo.exportedAt).toLocaleString("he-IL")}</strong>
                  </div>
                )}
                {backupInfo.counts && (
        ...
        {step === 3 && results && (
          <div style={cardStyle}>
            <div style={{ fontSize: 18, fontWeight: 700, color: "#6ee7b7", textAlign: "center" }}>✅ השחזור הושלם בהצלחה!</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6 }}>
            ...
            <a href="/" style={{ display: "block", textAlign: "center", padding: "12px", background: "linear-gradient(135deg,#7c3aed,#6d28d9)", color: "#fff", borderRadius: 10, fontSize: 14, fontWeight: 600, textDecoration: "none" }}>
              → חזור לאפליקציה
            </a>
          </div>
        )}
