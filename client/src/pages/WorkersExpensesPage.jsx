              ...
              {COLS.map(col => (
                <div key={col.key} style={{ ...CELL(col.width), color: item.colored ? "#991b1b" : "#374151" }}>
                  {isEditing ? (
                    ...
                    col.type==="boolean" ? (item[col.key] ? <span style={{ color:"#16a34a", fontWeight:600 }}>שולם</span> : <span style={{ color:"#ef4444", fontWeight:600 }}>לא שולם</span>) : col.type==="number" ? fmt(item[col.key]) : (item[col.key]||"-")
                  )}
                </div>
              ))}
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            ...
            <div>
              ...
              <input type="text" value={form.location} onChange={e => setForm(p => ({...p, location: e.target.value}))}  required={false}
                style={{ width:"100%", padding:"9px 12px", border:"1px solid #e5e7eb", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box", fontFamily:"inherit" }}
                onFocus={e => fo(e, theme.accent)} onBlur={bl} />
            </div>
            <div>
              <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#6b7280", marginBottom:6 }}>עבודה</label>
              <input type="text" value={form.equipment} onChange={e => setForm(p => ({...p, equipment: e.target.value}))}  required={false}
                style={{ width:"100%", padding:"9px 12px", border:"1px solid #e5e7eb", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box", fontFamily:"inherit" }}
                onFocus={e => fo(e, theme.accent)} onBlur={bl} />
            </div>
            <div>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, paddingTop:20 }}>
              <div style={{ position:"relative", width:36, height:20 }}>
                <div onClick={() => setForm(p => ({...p, tax: !p.tax}))} style={{ position:"absolute", inset:0, borderRadius:20, cursor:"pointer", background: form.tax ? theme.primary : "#d1d5db", transition:"0.2s" }}>
                  <div style={{ position:"absolute", top:2, right: form.tax ? 2 : 18, width:16, height:16, borderRadius:"50%", background:"#fff", transition:"0.2s" }} />
                </div>
              </div>
