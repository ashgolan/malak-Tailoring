...
import { workersExpensesApi, taxValuesApi } from "../api";
...
const COLS = [
  ...
  { key: "number",      label: "סכום",   type: "number",  width: "8%"  },
...
export default function WorkersExpensesPage() {
  ...
  const mobileCols = COLS.map(col => col.key === "tax"
    ? { ...col, render: (v) => v ? <span style={{ color:"#16a34a", fontWeight:600 }}>שולם</span> : <span style={{ color:"#ef4444", fontWeight:600 }}>לא שולם</span> }
    : col
  );
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20, direction:"rtl" }}>
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", flexWrap:"wrap", gap:12 }}>
        <div style={{ display:"flex", alignItems:"center", gap:12 }}>
          ...
          <div>
            <h1 style={{ fontSize:22, fontWeight:700, color:"#1f2937", margin:0 }}>הוצאות עובדים</h1>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            ...
            <div>
              ...
              <input type="text" list="workers-clients" autoComplete="off"
            ...
            <div>
              <label style={{ display:"block", fontSize:12, fontWeight:600, color:"#6b7280", marginBottom:6 }}>מיקום</label>
              <input type="text" list="workers-locations" autoComplete="off"
                value={editId ? editVals.location??""  : form.location}
                onChange={e => editId ? setEditVals(v=>({...v,location:e.target.value})) : setForm(p=>({...p,location:e.target.value}))}
                style={inputStyle} onFocus={e => fo(e, theme.accent)} onBlur={bl} />
              <datalist id="workers-locations">
            ...
            <div>
              ...
              <input type="text" list="workers-equipment" autoComplete="off"
                value={editId ? editVals.equipment??""  : form.equipment}
                onChange={e => editId ? setEditVals(v=>({...v,equipment:e.target.value})) : setForm(p=>({...p,equipment:e.target.value}))}
                style={inputStyle} onFocus={e => fo(e, theme.accent)} onBlur={bl} />
              <datalist id="workers-equipment">
                {allEquipment.map((e,i) => <option key={i} value={e} />)}
              </datalist>
            </div>
            ...
            <div style={{ display:"flex", alignItems:"center", gap:10, paddingTop:20 }}>
              <div style={{ position:"relative", width:36, height:20 }}>
                <div onClick={() => editId ? setEditVals(v=>({...v,tax:!v.tax})) : setForm(p=>({...p,tax:!p.tax}))}
                  style={{ position:"absolute", inset:0, borderRadius:20, cursor:"pointer", background: (editId?editVals.tax:form.tax) ? theme.primary : "#d1d5db", transition:"0.2s" }}>
                  <div style={{ position:"absolute", top:2, right: (editId?editVals.tax:form.tax) ? 2 : 18, width:16, height:16, borderRadius:"50%", background:"#fff", transition:"0.2s" }} />
                </div>
              </div>
              <label style={{ fontSize:13, fontWeight:600, color:"#374151" }}>שולם</label>
            </div>
