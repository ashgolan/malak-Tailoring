  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,0.45)", display:"flex", alignItems:"center", justifyContent:"center", zIndex:1100, padding:16 }}
      ...
      <div style={{ background:"#fff", borderRadius:16, width:"100%", maxWidth:520, maxHeight:"85vh", overflowY:"auto", padding:28, boxShadow:"0 20px 60px rgba(0,0,0,0.15)", direction:"rtl" }}>
          ...
          <div style={{ background:theme.primaryLight, borderRadius:12, padding:16, border:`1px solid ${theme.primaryBorder}` }}>
            ...
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:10, marginBottom:10 }}>
              <div>
                ...
                <input type="number" value={payForm.amount} min="0" max={balance} onChange={e => setPayForm(p=>({...p,amount:e.target.value}))}
              ...
              <div style={{ gridColumn:"1/-1" }}>
                ...
                <input type="text" value={payForm.note} placeholder="מזומן, העברה..." onChange={e => setPayForm(p=>({...p,note:e.target.value}))}
                  style={{ width:"100%", padding:"8px 10px", border:"1px solid #e5e7eb", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box", fontFamily:"inherit" }}
                  onFocus={e => fo(e, theme.accent)} onBlur={bl} />
              </div>
            </div>
            <button onClick={handleAdd} style={{ width:"100%", padding:"10px", border:"none", borderRadius:8, background:theme.gradient, color:"#fff", fontWeight:600, fontSize:13, cursor:"pointer", fontFamily:"inherit" }}>
              ✓ רשום תשלום
            </button>
          </div>
        )}
        ...
        <button onClick={onClose} style={{ width:"100%", marginTop:16, padding:"10px", border:"1px solid #e5e7eb", borderRadius:8, background:"#fff", fontSize:13, fontWeight:500, color:"#6b7280", cursor:"pointer", fontFamily:"inherit" }}>סגור</button>
      </div>
    </div>
  );
}
...
export default function PartialPaymentPage() {
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20, direction:"rtl" }}>
      ...
      {isMobile ? (
        <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
          {filtered.length === 0 ? (
            ...
            return (
              <div key={item._id} style={{ background: item.colored ? "#fef2f2" : balance <= 0 ? "#f0fdf4" : "#fff", borderRadius:12, border: item.colored ? "1px solid #fecaca" : balance <= 0 ? "1px solid #bbf7d0" : "1px solid #e5e7eb", padding:"12px 14px", boxShadow:"0 1px 4px rgba(0,0,0,0.05)" }}>
                ...
                <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:"6px 12px" }}>
                  <div><div style={{ fontSize:10, color:"#9ca3af", marginBottom:1 }}>קליינט</div><div style={{ fontSize:13, fontWeight:600, color:"#1f2937" }}>{item.clientName}</div></div>
                  <div><div style={{ fontSize:10, color:"#9ca3af", marginBottom:1 }}>עבור</div><div style={{ fontSize:13, fontWeight:500, color:"#1f2937", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{item.name}</div></div>
                  <div><div style={{ fontSize:10, color:"#9ca3af", marginBottom:1 }}>סכום כללי</div><div style={{ fontSize:13, fontWeight:600, color:theme.primary }}>{fmt(item.totalAmount)} ₪</div></div>
                  <div><div style={{ fontSize:10, color:"#9ca3af", marginBottom:1 }}>יתרה</div><div style={{ fontSize:15, fontWeight:700, color: balance > 0 ? "#ef4444" : "#16a34a" }}>{fmt(balance)} ₪</div></div>
                </div>
              </div>
            );
          })}
          {filtered.length > 0 && (
            <div style={{ background:theme.primaryLight, borderRadius:10, padding:"12px 16px", display:"flex", justifyContent:"space-between", border:`1px solid ${theme.primaryBorder}` }}>
              <span style={{ fontSize:13, color:"#6b7280" }}>יתרה כוללת</span>
            ...
            return (
              <div key={item._id} style={{ ...ROW, background:bg }}
                ...
                <div style={{ width:100, minWidth:100, padding:"10px 6px", display:"flex", gap:2, justifyContent:"center", flexShrink:0 }}>
                  <button onClick={() => setPaymentsModal(item)} style={{ padding:"3px 6px", background:theme.primaryLight, border:`1px solid ${theme.primaryBorder}`, borderRadius:6, color:theme.primary, cursor:"pointer", fontSize:11, fontWeight:600 }}>
                  ...
                  <div key={col.key} style={CELL(col.width, { color: item.colored ? "#991b1b" : "#374151" })}>
                    {col.key === "balance" ? (
                      <div style={{ display:"flex", alignItems:"center", gap:6 }}>
                        ...
                        {(item.payments||[]).length > 0 && <span style={{ fontSize:10, background:theme.primaryLight, color:theme.primary, padding:"1px 5px", borderRadius:10, fontWeight:600 }}>{(item.payments||[]).length} תשלומים</span>}
                      </div>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          ...
          {(Number(form.totalAmount) > 0 || Number(form.advanceAmount) > 0) && (
            <div style={{ background: Number(form.totalAmount)-Number(form.advanceAmount)>0 ? "#fef2f2" : "#f0fdf4", borderRadius:12, padding:"12px 16px", border:`1px solid ${Number(form.totalAmount)-Number(form.advanceAmount)>0 ? "#fecaca":"#bbf7d0"}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
              ...
              <span style={{ fontSize:22, fontWeight:700, color: Number(form.totalAmount)-Number(form.advanceAmount)>0 ? "#ef4444":"#16a34a" }}>{fmt(Number(form.totalAmount)-Number(form.advanceAmount))} ₪</span>
            </div>
          )}
      ...
      {paymentsModal && (
        <PaymentsModal item={paymentsModal} onClose={() => setPaymentsModal(null)} onAddPayment={handleAddPayment} theme={theme} />
      )}
    </div>
  );
}