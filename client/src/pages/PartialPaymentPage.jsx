...
export default function PartialPaymentPage() {
  ...
  const handleAddPayment = (id, payForm) => {
    ...
    const newP = [...(item.payments||[]), payForm];
    update(id,{payments:newP});
    setPaymentsModal(prev=>prev?{...prev,payments:newP}:null);
  };
        ...
        <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div>
              ...
              <input type="text" list="pp-clients"
              ...
              <datalist id="pp-clients">{allClients.map(c=><option key={c} value={c}/>)}</datalist>
            </div>
            <div style={{gridColumn:"1/-1"}}>
              <label style={S.label}>עבור</label>
              <input type="text" list="pp-names"
                ...
                required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/>
              <datalist id="pp-names">{allNames.map(n=><option key={n} value={n}/>)}</datalist>
            </div>
            <div>
