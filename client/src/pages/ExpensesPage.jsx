      ...
      <Modal isOpen={modal} onClose={()=>{setModal(false);setForm(EMPTY);setEditId(null);}} title={editId?"עריכת הוצאה":"הוספת הוצאה"} size="md">
        <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div style={{gridColumn:"1/-1"}}><label style={S.label}>שם / תיאור</label><input type="text" value={val("name")} onChange={e=>set("name",e.target.value)} list="exp-names" required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/><datalist id="exp-names">{allNames.map(n=><option key={n} value={n}/>)}</datalist></div>
