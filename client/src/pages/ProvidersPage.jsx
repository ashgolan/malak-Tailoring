  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20, direction:"rtl" }}>
      ...
      <div style={{ background:"#fff", borderRadius:16, border:"1px solid #f0f0ef", overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign:"center", padding:"56px 20px", color:"#9ca3af" }}><div style={{ fontSize:32, marginBottom:12 }}>🏭</div><div style={{ fontSize:15, fontWeight:500 }}>אין ספקים</div></div>
        ...
        {filtered.length > 0 && <div style={{ padding:"12px 16px", background:theme.primaryLight, borderTop:`2px solid ${theme.primaryBorder}` }}><span style={{ fontSize:13, color:"#6b7280" }}>{filtered.length} ספקים</span></div>}
      </div>

      <Modal isOpen={modal} onClose={() => { setModal(false); setForm(EMPTY); }} title="הוספת ספק">
