      ...
      <Modal isOpen={modal} onClose={()=>{setModal(false);setForm(EMPTY);setEditId(null);}} title={editId?"עריכת מכירה":"הוספת מכירה לחברה"} size="lg">
        <form onSubmit={handleSubmit} style={{display:"flex",flexDirection:"column",gap:14}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
            ...
            <div><label style={S.label}>חברה</label><input type="text" value={val("clientName")} onChange={e=>set("clientName",e.target.value)} list="stc-companies" required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/><datalist id="stc-companies">{allCompanies.map(c=><option key={c} value={c}/>)}</datalist></div>
            <div><label style={S.label}>עבודה</label><input type="text" value={val("name")} onChange={e=>set("name",e.target.value)} list="stc-works" required style={S.input} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/><datalist id="stc-works">{allWorks.map(w=><option key={w} value={w}/>)}</datalist></div>
            ...
            <div><label style={S.label}>סה״כ</label><input type="number" min="0" value={val("totalAmount")} onChange={e=>set("totalAmount",e.target.value)} style={{...S.input,fontWeight:700,color:theme.primary}} onFocus={e=>fo(e,theme.accent)} onBlur={bl}/></div>
