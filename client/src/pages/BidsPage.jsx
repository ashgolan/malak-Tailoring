...
// ─── BidModal ──────────────────────────────────────────────────
function BidModal({ initial, onClose, onSave, inventories, maam, theme, S }) {
    ...
    <div style={{position:"fixed",inset:0,background:"var(--bg-overlay)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16,backdropFilter:"blur(2px)"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"var(--bg-modal)",borderRadius:16,width:"100%",maxWidth:680,maxHeight:"90vh",overflowY:"auto",padding:28,boxShadow:"var(--shadow-modal)",direction:"rtl",border:"1px solid var(--border)"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:16}}>
          <div><label style={{display:"block",fontSize:12,fontWeight:600,color:"var(--text-3)",marginBottom:6}}>קליינט *</label><input style={inputS} value={form.clientName} onChange={set("clientName")} required onFocus={fo} onBlur={bl}/></div>
          <div><label style={{display:"block",fontSize:12,fontWeight:600,color:"var(--text-3)",marginBottom:6}}>תאריך</label><input type="date" style={inputS} value={form.date} onChange={set("date")} onFocus={fo} onBlur={bl}/></div>
          <div><label style={{display:"block",fontSize:12,fontWeight:600,color:"var(--text-3)",marginBottom:6}}>יעד / נושא</label><input style={inputS} value={form.target} onChange={set("target")} placeholder="עבור: כיסוי מחסן..." onFocus={fo} onBlur={bl}/></div>
        </div>
        ...
        {/* Free mode */}
        {bidType==="free"&&(<>
          <div style={{marginBottom:12}}><label style={{display:"block",fontSize:12,fontWeight:600,color:"var(--text-3)",marginBottom:6}}>תוכן ההצעה</label><textarea style={textareaS} value={freeText} onChange={e=>setFreeText(e.target.value)} placeholder="פרט את ההצעה כאן..." onFocus={fo} onBlur={bl}/></div>
          <div><label style={{display:"block",fontSize:12,fontWeight:600,color:"var(--text-3)",marginBottom:6}}>סכום כולל (₪)</label><input type="number" style={{...inputS,fontSize:18,fontWeight:700,color:theme.primary}} value={form.totalAmount} onChange={set("totalAmount")} onFocus={fo} onBlur={bl}/></div>
        </>)}
        <div style={{borderTop:"1px solid var(--border)",margin:"16px 0"}}/>
        {/* Approved toggle */}
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"var(--bg-overlay)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16,backdropFilter:"blur(2px)"}} onClick={e=>e.target===e.currentTarget&&onClose()}>
      <div style={{background:"var(--bg-modal)",borderRadius:16,width:"100%",maxWidth:560,maxHeight:"88vh",overflowY:"auto",padding:28,boxShadow:"var(--shadow-modal)",direction:"rtl",border:"1px solid var(--border)"}}>
        ...
        {isFree&&bid.data?.length>0&&(<div style={{background:"var(--bg-card-alt)",borderRadius:10,padding:"14px 16px",marginBottom:16,border:"1px solid var(--border)",whiteSpace:"pre-wrap",fontSize:13,lineHeight:1.8,color:"var(--text-1)"}}>{bid.data[0]?.text||""}</div>)}
        {!isFree&&bid.data?.length>0&&(
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,marginBottom:16}}>
            <thead><tr style={{background:theme.gradient,color:"#fff"}}>{["תיאור","כמות","מחיר","סה״כ"].map(h=><th key={h} style={{padding:"8px 10px",textAlign:"right",fontWeight:600,fontSize:12}}>{h}</th>)}</tr></thead>
            <tbody>{bid.data.map((row,i)=><tr key={i} style={{background:i%2===0?"var(--bg-card)":"var(--bg-card-alt)",borderBottom:"1px solid var(--border-light)"}}><td style={{padding:"8px 10px",color:"var(--text-1)"}}>{row.description}</td><td style={{padding:"8px 10px",textAlign:"center",color:"var(--text-2)"}}>{row.quantity}</td><td style={{padding:"8px 10px",textAlign:"right",color:"var(--text-2)"}}>{fmt(row.price)} ₪</td><td style={{padding:"8px 10px",textAlign:"right",fontWeight:700,color:theme.primary}}>{fmt(row.total)} ₪</td></tr>)}</tbody>
          </table>
        )}
...
export default function BidsPage() {
  ...
  return (
    <div style={{display:"flex",flexDirection:"column",gap:20,direction:"rtl"}}>
      ...
      {/* Stats */}
      <div style={S.statBar}>
        {[{label:'סה"כ לפני מע"מ',value:`${fmt(totalAll)} ₪`,color:"var(--text-1)"},{label:`מע"מ (${maam}%)`,value:`${fmt(taxTotal)} ₪`,color:"#d97706"},{label:'סה"כ כולל מע"מ',value:`${fmt(grandTotal)} ₪`,color:theme.primary}].map((stat,i)=>(
      ...
      {/* Cards */}
      {isLoading ? (
        <div style={{display:"flex",justifyContent:"center",padding:60}}><div className="rosh-spinner" style={{borderTopColor:theme.primary}}/></div>
      ) : filtered.length===0 ? (
        ...
        <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill, minmax(280px,1fr))",gap:14}}>
          {filtered.sort((a,b)=>a.date<b.date?1:-1).map(bid=>(
            <div key={bid._id} style={{background:"var(--bg-card)",borderRadius:14,border:`1px solid ${bid.isApproved?"rgba(22,163,74,0.3)":"var(--border-light)"}`,padding:"16px",boxShadow:"var(--shadow-card)",display:"flex",flexDirection:"column",gap:12,cursor:"pointer",transition:"transform 0.12s"}}
              ...
              {bid.target&&bid.target!=="-"&&<div style={{fontSize:13,color:"var(--text-2)",borderRight:`3px solid ${theme.primary}`,paddingRight:8}}>{bid.target}</div>}
              <div style={{fontSize:20,fontWeight:700,color:theme.primary}}>{fmt(bid.totalAmount)} ₪</div>
              <div style={{display:"flex",gap:8}} onClick={e=>e.stopPropagation()}>
                <button onClick={()=>setModal(bid)}
                  style={{flex:1,padding:"7px",border:"none",borderRadius:8,background:theme.primaryLight,color:theme.primary,fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit"}}>✎ עריכה</button>
                ...
                <button onClick={()=>{if(window.confirm("למחוק?"))remove(bid._id);}}
                  style={{padding:"7px 10px",border:"1px solid var(--border)",borderRadius:8,background:"var(--btn-cancel-bg)",fontSize:13,cursor:"pointer",color:"#ef4444"}}>🗑</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {modal&&<BidModal initial={modal==="add"?null:modal} onClose={()=>setModal(null)} onSave={handleSave} inventories={inventories} maam={maam} theme={theme} S={S}/>}
      {viewBid&&<ViewModal bid={viewBid} onClose={()=>setViewBid(null)} onToggleApprove={()=>toggleApprove(viewBid)} theme={theme}/>}
    </div>
  );
}
