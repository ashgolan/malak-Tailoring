...
export default function InstitutionTaxPage() {
  ...
  const { data: taxValues } = useQuery({ queryKey: ["settings"], queryFn: () => taxValuesApi.get().then(r => r.data) });
  ...
  // Unique institutions and works from existing data
  ...
  const set = (k, v) => setForm(p => {
    const updated = { ...p, [k]: v };
    // recalc totalAmount = number (same)
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:20, direction:"rtl" }}>
      ...
      <div style={{ background:"#fff", borderRadius:16, border:"1px solid #f0f0ef", overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
        ...
        {filtered.length === 0 ? (
          ...
          const masDeduction = calcMas(item.number || item.totalAmount);
          return (
      ...
      {/* Modal */}
      <Modal isOpen={modal} onClose={() => { setModal(false); setForm(EMPTY); }} title="הוספת חשבונית למוסד" size="md">
          ...
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            ...
            <div>
              ...
              <datalist id="institutions-list">
                {allInstitutions.map(i => <option key={i} value={i} />)}
              </datalist>
            </div>
            ...
            <div>
              ...
              <input type="number" value={form.number} min="0" onChange={e => set("number", e.target.value)}
          ...
          {Number(form.number) > 0 && (
            <div style={{ background:theme.primaryLight, border:`1px solid ${theme.primaryBorder}`, borderRadius:12, padding:"14px 18px" }}>
              <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, color:"#6b7280", marginBottom:6 }}>
                <span>סכום</span><span>{fmt(form.number)} ₪</span>
              </div>
              {masValue > 0 && (
                <div style={{ display:"flex", justifyContent:"space-between", fontSize:13, color:"#d97706", marginBottom:6 }}>
                  ...
                  <span>- {fmt(calcMas(form.number))} ₪</span>
                </div>
              )}
              <div style={{ borderTop:`1px solid ${theme.primaryBorder}`, paddingTop:10, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
                ...
                <span style={{ fontSize:20, fontWeight:700, color:theme.primary }}>{fmt(Number(form.number) - calcMas(form.number))} ₪</span>
              </div>
            </div>
          )}
