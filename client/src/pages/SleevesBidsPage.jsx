...
export default function SleevesBidsPage() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    const base = Number(form.number) * (form.quantity || 1); const finalForm = { ...form, totalAmount: form.tax ? base * (1 + maam/100) : base };
    create(finalForm);
  ...
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:20, direction:"rtl" }}>
      ...
      <div style={{ background:"#fff", borderRadius:16, border:"1px solid #f0f0ef", overflow:"hidden", boxShadow:"0 1px 4px rgba(0,0,0,0.04)" }}>
        ...
        {filtered.length === 0 ? (
          <div style={{ textAlign:"center", padding:"56px 20px", color:"#9ca3af" }}><div style={{ fontSize:32, marginBottom:12 }}>✂️</div><div style={{ fontSize:15, fontWeight:500 }}>אין נתונים להצגה</div></div>
        ...
        <form onSubmit={handleSubmit} style={{ display:"flex", flexDirection:"column", gap:14 }}>
          <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
            ...
            <div>
              ...
              <input type="number" value={form.quantity} onChange={e => setForm(p => ({...p, quantity: e.target.value}))} min="0" required={false}
                style={{ width:"100%", padding:"9px 12px", border:"1px solid #e5e7eb", borderRadius:8, fontSize:13, outline:"none", boxSizing:"border-box", fontFamily:"inherit" }}
                onFocus={e => fo(e, theme.accent)} onBlur={bl} />
            </div>
